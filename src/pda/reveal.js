...
import { canonicalizePythonJSON, parsePythonJSON, } from './canonical-json.js';
import { PDA_DOMAIN_CANDIDATE_COMMIT } from './constants.js';
 ...
 */
export async function verifyCandidateReveal(input) {
    const { pdaOutput, commitment, revealedSequence, revealedMetadataJSON, inclusionProof, } = input;
    ...
    if (!saltBytes)
        return false;
    const rebuiltLeaf = await pdaSha256(PDA_DOMAIN_CANDIDATE_COMMIT, saltBytes, sequenceBytes, metadataBytes);
 ...
 */
export async function computeCandidateCommitLeaf(saltHex, sequence, metadataJSON) {
 ...
 * hex digest.
 */
export async function computeCandidateMetadataHash(metadataJSON) {
    const parsed = parsePythonJSON(metadataJSON);
    const canonical = canonicalizePythonJSON(parsed);
...
function normalizePathEntry(entry) {
    ...
    if (typeof entry === 'object' &&
        entry !== null &&
        typeof entry.sibling_hex === 'string' &&
        (entry.side === 'left' ||
            entry.side === 'right')) {
        return entry;
    }
