...
import { canonicalizePythonJSON, } from './canonical-json.js';
import { PDA_DIGEST_LEN, PDA_DOMAIN_BIOSECURITY_POLICY, PDA_DOMAIN_PIPELINE_MANIFEST, PDA_DOMAIN_SEPARATOR, PDA_DOMAIN_TARGET_SPEC, PDA_DOMAIN_TEE_ATTESTATION, PDA_DOMAIN_TIER_DIST, PDA_SCHEMA_VERSION_V1, PDA_SIMULATOR_MEASUREMENT_HEX, PDA_SIMULATOR_SIGNING_KEY_HEX, } from './constants.js';
import { digestHex, parseDigestHex, pdaComponentHash, pdaHmacSha256, pdaSha256, pdaTimingSafeEqual, pdaU32BE, } from './hashing.js';
 ...
 */
export async function verifyPDA(output, options = {}) {
    const reasons = [];
    const verified = {};
    ...
    const expectedMeasurementHex = options.expectedMeasurementHex ?? PDA_SIMULATOR_MEASUREMENT_HEX;
    ...
    const seenNonces = new Set((options.seenNoncesHex ?? []).map((s) => s.toLowerCase()));
...
// Component hashes
// ---------------------------------------------------------------------------
async function hashTargetSpec(ts) {
    const payload = {
...
async function hashPipelineManifest(pm) {
    const payload = {
...
async function hashBiosecurityPolicy(bp) {
    const payload = {
...
async function hashTierDistribution(td) {
    const payload = {
...
async function hashTEEAttestation(tee) {
    const payload = {
...
function buildTeeTranscript(inputs) {
    const parts = [
...
async function buildPdaBytes(inputs) {
    ...
    return pdaSha256(PDA_DOMAIN_SEPARATOR, pdaU32BE(inputs.schemaVersion), inputs.targetSpecHash, inputs.pipelineManifestHash, inputs.biosecurityPolicyHash, inputs.candidateCommitRoot, inputs.tierDistributionHash, inputs.teeMeasurement, inputs.teeAttestationHash);
}
async function verifyTeeSignature(input) {
...
function report(passed, reasons, verified) {
    return {
        passed,
 ...
 * Rust references.
 */
export async function computePdaComponentHashes(output) {
    const targetSpecHash = await hashTargetSpec(output.target_spec);
    const pipelineManifestHash = await hashPipelineManifest(output.pipeline_manifest);
    const biosecurityPolicyHash = await hashBiosecurityPolicy(output.biosecurity_policy);
    const tierDistributionHash = await hashTierDistribution(output.tier_distribution);
 ...
 */
export async function recomputePdaHex(output, options = {}) {
    const schemaVersion = options.schemaVersion ?? PDA_SCHEMA_VERSION_V1;
    ...
    const measurementHex = options.expectedMeasurementHex ?? output.tee_attestation.measurement_hex;
