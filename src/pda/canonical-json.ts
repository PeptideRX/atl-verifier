...
/**
 * A JSON value that may also carry {@link PyFloatNumber} markers for
 * preserved float literals. Returned by {@link parsePythonJSON} and
 * accepted by {@link canonicalizePythonJSON}.
 */
...
/**
 * Tagged wrapper for a Python float literal.
 *
 * The wire representation of every float that passes through the PDA
 * canonical JSON layer is preserved verbatim in {@link literal}. This
 * includes the optional leading `-`, the mandatory decimal point (or
 * exponent marker), and the digits on either side.
 *
 * Constructed exclusively by {@link parsePythonJSON}. User code that
 * wants to emit a float without a `.` in the parsed source may use
 * {@link asPythonFloat}.
 */
export interface PyFloatNumber {
  readonly __jcsFloat: true;
 ...
 */
export function isPyFloatNumber(v: unknown): v is PyFloatNumber {
  return (
    typeof v === 'object' &&
    v !== null &&
    (v as { __jcsFloat?: unknown }).__jcsFloat === true &&
 ...
 */
export function asPythonFloat(n: number): PyFloatNumber {
 ...
 */
export function parsePythonJSON(src: string): PyJsonValue {
 ...
 */
export function canonicalizePythonJSON(value: PyJsonValue): string {
  return emit(value);
}
...
function emitArray(arr: unknown[]): string {
...
function emitObject(obj: Record<string, unknown>): string {
...
function compareUtf8(a: string, b: string): number {
  ...
  for (let i = 0; i < n; i++) {
    const da = ab[i] as number;
    const db = bb[i] as number;
...
class Parser {
  ...
  parseObject(): { [key: string]: PyJsonValue } {
    this.i++; // consume '{'
    const out: { [key: string]: PyJsonValue } = {};
    this.skipWhitespace();
    ...
    for (;;) {
      ...
      if (this.src.charAt(this.i) !== '"') {
        throw new SyntaxError(
          `parsePythonJSON: expected string key at offset ${this.i}`,
        );
      }
      const key = this.parseString();
      if (Object.prototype.hasOwnProperty.call(out, key)) {
        throw new SyntaxError(
          `parsePythonJSON: duplicate key "${key}" at offset ${this.i}`,
        );
      }
      this.skipWhitespace();
      if (this.src.charAt(this.i) !== ':') {
        throw new SyntaxError(
          `parsePythonJSON: expected ':' at offset ${this.i}`,
        );
      }
      this.i++;
      ...
      throw new SyntaxError(
        `parsePythonJSON: expected ',' or '}' at offset ${this.i}`,
      );
    }
  }

  parseArray(): PyJsonValue[] {
    this.i++; // consume '['
    const out: PyJsonValue[] = [];
    this.skipWhitespace();
    if (this.src.charAt(this.i) === ']') {
      this.i++;
      return out;
  ...
  parseString(): string {
    if (this.src.charAt(this.i) !== '"') {
      throw new SyntaxError(
        `parsePythonJSON: expected string at offset ${this.i}`,
      );
    }
    this.i++;
    let out = '';
    while (this.i < this.src.length) {
      ...
      if (ch === '\\') {
        ...
        switch (esc) {
          ...
          case 'u': {
            ...
            if (!/^[0-9a-fA-F]{4}$/.test(hex)) {
              throw new SyntaxError(
                `parsePythonJSON: invalid \\u escape at offset ${this.i}`,
              );
            }
            this.i += 4;
          ...
          default:
            throw new SyntaxError(
              `parsePythonJSON: invalid escape \\${esc} at offset ${this.i}`,
            );
        }
      } else {
        ...
        if (code < 0x20) {
          throw new SyntaxError(
            `parsePythonJSON: control character in string at offset ${this.i}`,
          );
        }
        out += ch;
        this.i++;
  ...
  parseBool(): boolean {
    ...
    throw new SyntaxError(
      `parsePythonJSON: expected bool at offset ${this.i}`,
    );
  }
  ...
  parseNull(): null {
    ...
    throw new SyntaxError(
      `parsePythonJSON: expected null at offset ${this.i}`,
    );
  }

  parseNumber(): number | PyFloatNumber {
    const start = this.i;
    if (this.src.charAt(this.i) === '-') this.i++;
    if (this.src.charAt(this.i) === '0') {
      ...
      throw new SyntaxError(
        `parsePythonJSON: expected digit at offset ${this.i}`,
      );
    }
    let isFloat = false;
    if (this.src.charAt(this.i) === '.') {
      ...
      if (digits === 0) {
        throw new SyntaxError(
          `parsePythonJSON: expected digits after '.' at offset ${this.i}`,
        );
      }
    }
    if (this.src.charAt(this.i) === 'e' || this.src.charAt(this.i) === 'E') {
      ...
      if (digits === 0) {
        throw new SyntaxError(
          `parsePythonJSON: expected exponent digits at offset ${this.i}`,
        );
      }
    }
    ...
    if (!Number.isFinite(n)) {
      throw new SyntaxError(
        `parsePythonJSON: non-finite integer literal "${literal}"`,
      );
    }
    return n;
  }
