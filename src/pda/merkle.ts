 ...
 */
export async function pdaMerkleRoot(leaves: readonly Uint8Array[]): Promise<Uint8Array> {
  if (leaves.length === 0) {
  ...
  for (let i = 0; i < leaves.length; i++) {
    ...
    if (!(leaf instanceof Uint8Array) || leaf.length !== PDA_DIGEST_LEN) {
      throw new Error(
        `pdaMerkleRoot: leaf ${i} must be exactly ${PDA_DIGEST_LEN} bytes`,
      );
    }
  }
  let level: Uint8Array[] = leaves.map((l) => new Uint8Array(l));
  while (level.length > 1) {
    if (level.length % 2 === 1) {
      const last = level[level.length - 1] as Uint8Array;
 ...
 */
export async function pdaVerifyInclusionProof(
  leaf: Uint8Array,
  path: ReadonlyArray<readonly [Uint8Array, 'left' | 'right']>,
  expectedRoot: Uint8Array,
): Promise<boolean> {
  if (!(leaf instanceof Uint8Array) || leaf.length !== PDA_DIGEST_LEN) return false;
  if (
 ...
 * one-leaf tree.
 */
export async function pdaGenerateInclusionProof(
  leaves: readonly Uint8Array[],
  leafIndex: number,
): Promise<Array<readonly [Uint8Array, 'left' | 'right']>> {
  if (leafIndex < 0 || leafIndex >= leaves.length) {
    throw new RangeError(
      `pdaGenerateInclusionProof: leafIndex ${leafIndex} out of range for ${leaves.length} leaves`,
    );
  }
  for (let i = 0; i < leaves.length; i++) {
    const leaf = leaves[i];
  ...
  let level: Uint8Array[] = leaves.map((l) => new Uint8Array(l));
  let idx = leafIndex;
  const proof: Array<readonly [Uint8Array, 'left' | 'right']> = [];
  while (level.length > 1) {
    if (level.length % 2 === 1) {
      const last = level[level.length - 1] as Uint8Array;
      level.push(new Uint8Array(last));
    ...
    if (idx % 2 === 0) {
      const sibling = level[idx + 1] as Uint8Array;
...
async function hashInternal(left: Uint8Array, right: Uint8Array): Promise<Uint8Array> {
  if (left.length !== PDA_DIGEST_LEN || right.length !== PDA_DIGEST_LEN) {
    throw new Error(
      `pdaMerkle hashInternal: children must be ${PDA_DIGEST_LEN} bytes each`,
    );
  }
